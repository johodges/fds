...
INTEGER :: I,J,K,NS,NR,N,CHEM_SUBIT_TMP, ICC, JCC, NCELL,RICH_ITER_MAX
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM
...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TMP_IN,RHO_IN,PRES_IN,MU_IN,DELTA,CELL_VOLUME,UNMIXED_FRACTION,IIC,JJC,KKC)
...
REAL(EB), INTENT(INOUT), OPTIONAL :: UNMIXED_FRACTION
REAL(EB) :: A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            ZZ_MIXED_NEW(1:N_TRACKED_SPECIES),TAU_MIX,DT_SUB_MIN,RHO_HAT,&
            ...
            ZETA_1,ZETA_2,ZETA_4,TMP_IN,TMP_IN2,&
            DT_SUB_OLD,TNOW2,ERR_EST(N_TRACKED_SPECIES),ERR_TOL(N_TRACKED_SPECIES),ERR_TINY,&
            ZZ_TEMP(1:N_TRACKED_SPECIES),H_FACTOR
...
ZETA_0 = UNMIXED_FRACTION
CALL GET_MIXING_TIME(TMP_IN, MU_IN, RHO_IN, DELTA, MIX_TIME_OUT)
   ...
   INTEGRATOR_SELECT: SELECT CASE (COMBUSTION_ODE_SOLVER)
      ...
      CASE (RK2_RICHARDSON) ! Finite-rate (or mixed finite-rate/fast) chemistry
         ...
         RICH_EX_LOOP: DO RICH_ITER = 1,RICH_ITER_MAX
            ...
            CALL FIRE_RK2(A1,ZZ_MIXED,ZZ_0,ZETA_1,ZETA_0,DT_SUB,1,TMP_IN,RHO_HAT,CELL_MASS,&
                        Q_REAC_1,TOTAL_MIXED_MASS_1,NO_REACTIONS)
            IF (NO_REACTIONS) EXIT RICH_EX_LOOP
            CALL FIRE_RK2(A2,ZZ_MIXED,ZZ_0,ZETA_2,ZETA_0,DT_SUB,2,TMP_IN,RHO_HAT,CELL_MASS,&
                        Q_REAC_2,TOTAL_MIXED_MASS_2,NO_REACTIONS)
            CALL FIRE_RK2(A4,ZZ_MIXED,ZZ_0,ZETA_4,ZETA_0,DT_SUB,4,TMP_IN,RHO_HAT,CELL_MASS,&
...
IF (SUPPRESSION .AND. .NOT.EXTINCT) THEN
   SELECT CASE(EXTINCT_MOD)
      ...
      CASE(EXTINCTION_2); 
         TMP_IN2 = TMP_IN
         CALL EXTINCT_2(EXTINCT,ZZ_0,ZZ_MIXED,TMP_IN2,H_FACTOR)
      CASE(EXTINCTION_3); 
         TMP_IN2 = (Q_OUT*CHI_R_OUT/(4.*SIGMA))**0.25_EB !TMP_IN*2 !*RTE_SOURCE_CORRECTION_FACTOR**0.25_EB
         TMP_IN2 = ((Q_OUT*CHI_R_OUT+KAPPA_GAS(IIC,JJC,KKC))/(4.*KAPPA_GAS(IIC,JJC,KKC)*SIGMA))**0.25_EB 
         !WRITE(*,*) "TMP_IN", TMP_IN, "TMP_IN2", TMP_IN2, "Q_OUT", Q_OUT, "CHI_R_OUT", CHI_R_OUT, "UII", &
         !UII(IIC,JJC,KKC), "KAPPA", KAPPA_GAS(IIC,JJC,KKC)
         TMP_IN2 = MAX(TMP_IN,TMP_IN2)
         CALL EXTINCT_2(EXTINCT,ZZ_0,ZZ_MIXED,TMP_IN2,H_FACTOR)
         IF ((H_FACTOR > TWO_EPSILON_EB).AND.(H_FACTOR < 1._EB)) THEN 
            EXTINCT = .FALSE.
            !IF (ANY(Q_REAC_SUM>TWO_EPSILON_EB)) THEN
               DO NR=1,N_REACTIONS
                  Q_REAC_SUM(NR) = Q_REAC_SUM(NR)*H_FACTOR
               ENDDO
               !WRITE(*,*) "Q_OUT1", Q_OUT, "H_FACTOR", H_FACTOR
               Q_OUT = -RHO_IN*SUM(SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%H_F*(ZZ_GET-ZZ_0))/DT ! FDS Tech Guide (5.47)
               DO NS = 1,N_TRACKED_SPECIES
                  ZZ_GET(NS) = (ZZ_GET(NS)-ZZ_0(NS))*H_FACTOR + ZZ_0(NS)
               ENDDO
               !WRITE(*,*) " Q_OUT2", Q_OUT
            !ENDIF
         ENDIF
...
SUBROUTINE GET_MIXING_TIME(TMP_IN, MU_IN, RHO_IN, DELTA, MIX_TIME_OUT)
REAL(EB), INTENT(OUT) :: MIX_TIME_OUT
REAL(EB), INTENT(IN) :: DELTA,MU_IN,RHO_IN,TMP_IN
REAL(EB) :: D_F,C_U,TAU_D,TAU_G,TAU_U
INTEGER :: NR
TYPE(REACTION_TYPE), POINTER :: RN=>NULL() !,R1=>NULL()
...
END SUBROUTINE GET_MIXING_TIME
...
SUBROUTINE EXTINCT_2(EXTINCT,ZZ_0,ZZ_IN,TMP_IN2,H_FACTOR)
...
IF (EXTINCT) THEN
   H_FACTOR = 0._EB
   CALL GET_ENTHALPY(ZZ_HAT,H_MIN,MAX(CFT-EXT_MIN_TEMP_OFFSET,273.15_EB)) ! H of products at the offset critical flame temperature
   !H_MIN = MAX(0._EB,H_MIN)
   !IF (H_CRIT > TWO_EPSILON_EB) THEN
   !   IF (H_0 > TWO_EPSILON_EB) THEN
   ...
   !ENDIF
   H_FACTOR = MAX(0._EB,MIN(1._EB-TWO_EPSILON_EB, H_FACTOR))
ELSE
   H_FACTOR = 1._EB
ENDIF
...
SUBROUTINE FIRE_FORWARD_EULER(ZZ_OUT,ZZ_IN,ZZ_0,ZETA_OUT,ZETA_IN,DT_LOC,TMP_IN,RHO_HAT,CELL_MASS,&
                              Q_REAC_LOC,TOTAL_MIXED_MASS,NO_REACTIONS)
USE PHYSICAL_FUNCTIONS, ONLY: GET_REALIZABLE_MF,GET_AVERAGE_SPECIFIC_HEAT
REAL(EB), INTENT(IN) :: ZZ_0(1:N_TRACKED_SPECIES),ZZ_IN(1:N_TRACKED_SPECIES),ZETA_IN,DT_LOC,RHO_HAT,CELL_MASS
...
SUBROUTINE FIRE_RK2(ZZ_OUT,ZZ_IN,ZZ_0,ZETA_OUT,ZETA_IN,DT_SUB,N_INC,TMP_IN,RHO_HAT,CELL_MASS,&
...
DO N=1,N_INC
   CALL FIRE_FORWARD_EULER(ZZ_TMP_1,ZZ_TMP_0,ZZ_0,ZETA_TMP_1,ZETA_TMP_0,DT_LOC,TMP_IN,RHO_HAT,CELL_MASS,&
                           Q_REAC_1,TOTAL_MIXED_MASS_1,NO_REACTIONS)

   CALL FIRE_FORWARD_EULER(ZZ_TMP_2,ZZ_TMP_1,ZZ_0,ZETA_TMP_2,ZETA_TMP_1,DT_LOC,TMP_IN,RHO_HAT,CELL_MASS,&
                           Q_REAC_2,TOTAL_MIXED_MASS_2,NO_REACTIONS)
