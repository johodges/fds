...
REAL(EB) :: A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            ZETA_1,ZETA_2,ZETA_4,D_F,TMP_IN,TMP_IN2,&
            C_U,DT_SUB_OLD,TNOW2,ERR_EST(N_TRACKED_SPECIES),ERR_TOL(N_TRACKED_SPECIES),ERR_TINY,&
...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_SUBSTEPS
   ...
   IF (SUPPRESSION) THEN
      DO NR=1,N_REACTIONS
         RN=>REACTION(NR)
         IF (ZZ_0(RN%FUEL_SMIX_INDEX)>1E-4 .AND. ZZ_0(RN%AIR_SMIX_INDEX)>TWO_EPSILON_EB) THEN
...
! get CHI_R_OUT before checking extinction for flame temperature correction
...
IF (SUPPRESSION .AND. .NOT.EXTINCT) THEN
   SELECT CASE(EXTINCT_MOD)
      CASE(EXTINCTION_1);
         CALL EXTINCT_1(EXTINCT,ZZ_0,TMP_IN)
      CASE(EXTINCTION_2); 
         TMP_IN2 = TMP_IN
         CALL EXTINCT_2(EXTINCT,ZZ_0,ZZ_MIXED,TMP_IN2)
      CASE(EXTINCTION_3); 
         !TMP_IN2 = (Q_OUT*CHI_R_OUT/(4.*SIGMA))**0.25_EB !TMP_IN*2 !*RTE_SOURCE_CORRECTION_FACTOR**0.25_EB
         !WRITE(*,*) "TMP_IN", TMP_IN, "TMP_IN2", TMP_IN2, "Q_OUT", Q_OUT, "CHI_R_OUT", CHI_R_OUT, "SIGMA", SIGMA
         !TMP_IN2 = MAX(TMP_IN,TMP_IN2)
         CALL EXTINCT_2(EXTINCT,ZZ_0,ZZ_MIXED,TMP_IN)
         IF (H_FACTOR > TWO_EPSILON_EB) THEN 
            EXTINCT = .FALSE.
            IF (ANY(Q_REAC_SUM>TWO_EPSILON_EB)) THEN
               DO NR=1,N_REACTIONS
                  Q_REAC_SUM(NR) = Q_REAC_SUM(NR)*H_FACTOR
               ENDDO
            ENDIF
         ENDIF
   END SELECT
...
SUBROUTINE EXTINCT_2(EXTINCT,ZZ_0,ZZ_IN,TMP_IN2)

USE PHYSICAL_FUNCTIONS, ONLY: GET_ENTHALPY
REAL(EB),INTENT(IN) :: TMP_IN2,ZZ_IN(1:N_TRACKED_SPECIES),ZZ_0(1:N_TRACKED_SPECIES)
LOGICAL, INTENT(INOUT) :: EXTINCT
REAL(EB), INTENT(OUT) :: H_FACTOR
REAL(EB) :: ZZ_HAT_0(1:N_TRACKED_SPECIES),ZZ_HAT(1:N_TRACKED_SPECIES),H_0,H_CRIT,PHI_TILDE,CFT,H_DELTA
INTEGER :: NS,NR
...
IF (.NOT.EXTINCT) THEN
   H_FACTOR = 1._EB
ELSE
   H_DELTA = (H_0 - H_CRIT)/50._EB
   H_FACTOR = (EXP(H_DELTA)-EXP(-H_DELTA))/(EXP(H_DELTA)+EXP(-H_DELTA))/2._EB + 0.5_EB
ENDIF
