...
NAMELIST /TIME/ DT,DT_END_FILL,DT_END_MINIMUM,DT_EXTERNAL,FYI,LIMITING_DT_RATIO,LOCK_TIME_STEP,&
                RESTRICT_TIME_STEP,T_BEGIN,T_END,TIME_SHRINK_FACTOR,WALL_INCREMENT, T_END_CLOCK
...
NAMELIST /DUMP/ CFL_FILE,CLIP_RESTART_FILES,COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,DEVC_COLUMN_LIMIT,&
                ...
                STATUS_FILES,SUPPRESS_DIAGNOSTICS,TURB_INIT_CLOCK,VELOCITY_ERROR_FILE,VTK_BINARY,WRITE_XYZ,WRITE_STL
...
IF (DT_VTK_SPECIFIED > 0) THEN ; PARAVIEW_PROJECT=.TRUE.; ENDIF
...
REAL(EB) :: AGL_SLICE
INTEGER :: N,NN,NM,MESH_NUMBER,N_SLCF_O,NITER,ITER,VELO_INDEX,GEOM_INDEX,N_UNIQUE_SLCF
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   IF (.NOT.ALLOCATED(UNIQUE_SLICE_NAMES)) THEN
      ALLOCATE(UNIQUE_SLICE_NAMES(N_SLCF_O*3))
   ENDIF
   CALL COUNT_UNIQUE_SLCF(N_SLCF_O,N_UNIQUE_SLCF,UNIQUE_SLICE_NAMES)
   ...
   IF (.NOT.ALLOCATED(MESHES(1)%ALL_SLICE_NAMES)) THEN
      ALLOCATE(MESHES(1)%ALL_SLICE_NAMES(N_SLCF_O*3))
      ALLOCATE(MESHES(1)%ALL_SLICE_QUANTITIES(N_SLCF_O*3))
      DO NN=1,N_SLCF_O*3
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      CALL GET_SLCF_NAME(PBX,PBY,PBZ,XS_MIN,XF_MAX,YS_MIN,YF_MAX,ZS_MIN,ZF_MAX,SLCF_NAME)
      
      MESHES(1)%ALL_SLICE_NAMES(NN) = TRIM(SLCF_NAME)
      ...
      MESHES(1)%ALL_SLICE_QUANTITIES(NN) = TRIM(SL%SMOKEVIEW_LABEL(1:30))
   ENDDO SLCF_LOOP
...
IF (PBX>-1.E5_EB .OR. PBY>-1.E5_EB .OR. PBZ>-1.E5_EB) THEN
   IF (PBX>-1.E5_EB) THEN 
      WRITE(SLCF_NAME,'(A,I6.6,I2.2)') 'X_',NINT(PBX),NINT(ABS(NINT(PBX)-PBX))*100
   ELSEIF (PBY>-1.E5_EB) THEN
      WRITE(SLCF_NAME,'(A,I6.6,I2.2)') 'Y_',NINT(PBY),NINT(ABS(NINT(PBY)-PBY))*100
   ELSEIF (PBZ>-1.E5_EB) THEN 
      WRITE(SLCF_NAME,'(A,I6.6,I2.2)') 'Z_',NINT(PBZ),NINT(ABS(NINT(PBZ)-PBZ))*100
   ENDIF
   
ELSE
   WRITE(XNAME,'(I6.6,I2.2,A,I6.6,I2.2,A)') NINT(XS_MIN),NINT(ABS(NINT(XS_MIN)-XS_MIN))*100,'_',&
                                                  NINT(XF_MAX),NINT(ABS(NINT(XF_MAX)-XF_MAX))*100,'_'
   WRITE(YNAME,'(I6.6,I2.2,A,I6.6,I2.2,A)') NINT(YS_MIN),NINT(ABS(NINT(YS_MIN)-YS_MIN))*100,'_',&
                                                  NINT(YF_MAX),NINT(ABS(NINT(YF_MAX)-YF_MAX))*100,'_'
   WRITE(ZNAME,'(I6.6,I2.2,A,I6.6,I2.2)') NINT(ZS_MIN),NINT(ABS(NINT(ZS_MIN)-ZS_MIN))*100,'_',&
                                                NINT(ZF_MAX),NINT(ABS(NINT(ZF_MAX)-ZF_MAX))*100
   WRITE(SLCF_NAME,'(A,A,A,A)') 'SL3D_',TRIM(XNAME),TRIM(YNAME),TRIM(ZNAME)
ENDIF

END SUBROUTINE GET_SLCF_NAME

SUBROUTINE COUNT_UNIQUE_SLCF(N_SLCF_O,N_UNIQUE_SLCF,UNIQUE_SLICE_NAMES)
