...
USE COMP_FUNCTIONS, ONLY : SEARCH_INPUT_FILE
USE MATH_FUNCTIONS, ONLY : GET_RAMP_INDEX
CHARACTER(LABEL_LENGTH) :: CONDUCTIVITY_RAMP,CONDUCTIVITY_RAMP_X,CONDUCTIVITY_RAMP_Y,CONDUCTIVITY_RAMP_Z,&
                           SPECIFIC_HEAT_RAMP
CHARACTER(LABEL_LENGTH) :: SPEC_ID(MAX_SPECIES,MAX_REACTIONS),PART_ID(MAX_LPC,MAX_REACTIONS)
REAL(EB) :: EMISSIVITY,CONDUCTIVITY,CONDUCTIVITY_X,CONDUCTIVITY_Y,CONDUCTIVITY_Z,&
            SPECIFIC_HEAT,DENSITY,ABSORPTION_COEFFICIENT,BOILING_TEMPERATURE, &
...
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,ADJUST_H,ALLOW_SHRINKING,ALLOW_SWELLING,BOILING_TEMPERATURE,&
                ...
                CONDUCTIVITY_RAMP_X,CONDUCTIVITY_RAMP_Y,CONDUCTIVITY_RAMP_Z,&
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   ML%K_S                              = CONDUCTIVITY
   ALLOCATE(ML%K_S_X(0:I_MAX_TEMP),STAT=IZERO)
   CALL ChkMemErr('READ','ML%K_S_X',IZERO)
   IF (CONDUCTIVITY_X <= 0._EB) CONDUCTIVITY_X = CONDUCTIVITY
   IF (CONDUCTIVITY_Y <= 0._EB) CONDUCTIVITY_Y = CONDUCTIVITY
   IF (CONDUCTIVITY_Z <= 0._EB) CONDUCTIVITY_Z = CONDUCTIVITY
   ML%K_S_X                            = CONDUCTIVITY_X
   ALLOCATE(ML%K_S_Y(0:I_MAX_TEMP),STAT=IZERO)
   CALL ChkMemErr('READ','ML%K_S_Y',IZERO)
   ML%K_S_Y                            = CONDUCTIVITY_Y
   ALLOCATE(ML%K_S_Z(0:I_MAX_TEMP),STAT=IZERO)
   CALL ChkMemErr('READ','ML%K_S_Z',IZERO)
   ML%K_S_Z                            = CONDUCTIVITY_Z
   ...
   ML%RAMP_K_S_X                       = CONDUCTIVITY_RAMP_X
   ML%RAMP_K_S_Y                       = CONDUCTIVITY_RAMP_Y
   ML%RAMP_K_S_Z                       = CONDUCTIVITY_RAMP_Z
   ...
   IF (ML%RAMP_K_S/='null') THEN
      CALL GET_RAMP_INDEX(ML%RAMP_K_S,'TEMPERATURE',ML%I_RAMP_K_S)
      CALL GET_RAMP_INDEX(ML%RAMP_K_S,'TEMPERATURE',ML%I_RAMP_K_S_X)
      CALL GET_RAMP_INDEX(ML%RAMP_K_S,'TEMPERATURE',ML%I_RAMP_K_S_Y)
      CALL GET_RAMP_INDEX(ML%RAMP_K_S,'TEMPERATURE',ML%I_RAMP_K_S_Z)
   ENDIF
   IF (ML%RAMP_K_S_X/='null') CALL GET_RAMP_INDEX(ML%RAMP_K_S_X,'TEMPERATURE',ML%I_RAMP_K_S_X)
   IF (ML%RAMP_K_S_Y/='null') CALL GET_RAMP_INDEX(ML%RAMP_K_S_Y,'TEMPERATURE',ML%I_RAMP_K_S_Y)
   IF (ML%RAMP_K_S_Z/='null') CALL GET_RAMP_INDEX(ML%RAMP_K_S_Z,'TEMPERATURE',ML%I_RAMP_K_S_Z)
...
CONDUCTIVITY_RAMP_X    = 'null'
CONDUCTIVITY_RAMP_Y    = 'null'
CONDUCTIVITY_RAMP_Z    = 'null'
CONDUCTIVITY_X         = 0.0_EB      ! W/m/K
CONDUCTIVITY_Y         = 0.0_EB      ! W/m/K
CONDUCTIVITY_Z         = 0.0_EB      ! W/m/K
DENSITY                = 0._EB       ! kg/m3
E                      = -1._EB      ! J/mol
EMISSIVITY             = 0.9_EB
FYI                    = 'null'
