...
IF_HT3D: IF (N_HT3D_SURF_LINES>0) THEN
   ...
   DO I=1,N_HT3D_OBST
      ...
      IF (.NOT.ALLOCATED(MC%LIST)) THEN
         ALLOCATE(MC%LIST(NMESHES)) ; MC%LIST = 0
      ENDIF
...
DO NM=1,NMESHES
   ...
   DO I=1,N_GROUPS
      MC => MESH_COMM(I)
      IF (.NOT.ALLOCATED(MC%LIST)) CYCLE
...
IF (ALLOCATED(MESH_COMM)) DEALLOCATE(MESH_COMM)
...
CHARACTER(LABEL_LENGTH) :: WRITE_FORMAT
NAMELIST /DUMP/ BINGEOM_DIR,CFL_FILE,CLIP_RESTART_FILES,COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,DEVC_COLUMN_LIMIT,&
                ...
                VTK_COMPRESSION_LEVEL,VTK_DIR,VTK_KEEPOPEN,WRITE_FORMAT,&
...
! Select the output format(s) to write

IF (VTK_COMPRESSION_LEVEL<0 .OR. VTK_COMPRESSION_LEVEL>9) THEN
   WRITE(MESSAGE,'(A,I0,A)') 'ERROR(101): VTK_COMPRESSION_LEVEL, ',VTK_COMPRESSION_LEVEL,', must be between 0 and 9.'
   CALL SHUTDOWN(MESSAGE) ; RETURN
ENDIF

SELECT CASE(WRITE_FORMAT)
   CASE('SMV')
      WRITE_SMV = .TRUE.  ; WRITE_VTK = .FALSE.
   CASE('VTK')
      WRITE_SMV = .FALSE. ; WRITE_VTK = .TRUE.
   CASE('BOTH')
      WRITE_SMV = .TRUE.  ; WRITE_VTK = .TRUE.
   CASE DEFAULT
      WRITE(MESSAGE,'(A,A,A)') 'ERROR(101): WRITE_FORMAT, ',TRIM(WRITE_FORMAT),', is not recognized.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
END SELECT
...
! DT_VTK sets every VTKHDF output interval at once; otherwise each VTKHDF output follows
! its Smokeview counterpart's interval.

IF (DT_VTK_SPECIFIED > 0._EB) WRITE_VTK = .TRUE.
IF (WRITE_VTK) THEN
   PARAVIEW_PROJECT = .TRUE.
   WRITE_VTK_GEOM   = .TRUE.
   IF (DT_VTK_SPECIFIED > 0._EB) THEN
   ...
   ELSE
      IF (DT_SLCF_SPECIFIED    > 0._EB) THEN ; DT_SLCF_VTK = DT_SLCF_SPECIFIED    ; ELSE ; DT_SLCF_VTK = -1._EB ; ENDIF
      IF (DT_SL3D_SPECIFIED    > 0._EB) THEN ; DT_SL3D_VTK = DT_SL3D_SPECIFIED    ; ELSE ; DT_SL3D_VTK = -1._EB ; ENDIF
      IF (DT_SMOKE3D_SPECIFIED > 0._EB) THEN ; DT_SM3D_VTK = DT_SMOKE3D_SPECIFIED ; ELSE ; DT_SM3D_VTK = -1._EB ; ENDIF
      IF (DT_BNDF_SPECIFIED    > 0._EB) THEN ; DT_BNDF_VTK = DT_BNDF_SPECIFIED    ; ELSE ; DT_BNDF_VTK = -1._EB ; ENDIF
      IF (DT_PART_SPECIFIED    > 0._EB) THEN ; DT_PART_VTK = DT_PART_SPECIFIED    ; ELSE ; DT_PART_VTK = -1._EB ; ENDIF
   ENDIF
ENDIF
...
! Check for custom directory for VTKHDF output.  Defaults to RESULTS_DIR.
...
IF (WRITE_VTK .AND. VTK_DIR/=RESULTS_DIR) THEN
   ! Make the directory on every rank, so that a rank that reaches its first output
   ! before rank 0 does not fail writing into a directory that does not exist yet.
#ifdef _WIN32
...
! The ParaView state script needs an absolute path to the output files

IF (PARAVIEW_PROJECT .AND. MY_RANK==0) THEN
#ifdef _WIN32
...
INTEGER  :: N,NN,NM,MESH_NUMBER=0,N_DEVC_READ,IOR,TRIP_DIRECTION,VELO_INDEX,POINTS,I_POINT,PIPE_INDEX,ORIENTATION_INDEX, &
            N_INTERVALS,MOVE_INDEX,IIG,JJG,KKG,NOM,ERROR_CODE,LP_TAG,ND
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   MESH_LOOP_2: DO NM=1,NMESHES
      ...
      IF (DV%X1>-1.E5_EB) THEN
         ...
         SEARCH_LOOP: DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
            IF (DV%QUANTITY(1)==OUTPUT_QUANTITY(ND)%NAME) THEN
               IF (OUTPUT_QUANTITY(ND)%CELL_POSITION==CELL_CENTER) THEN  ! do not allow zero for a cell index
                  SDV%I2 = MAX(SDV%I2,1)
                  SDV%J2 = MAX(SDV%J2,1)
                  SDV%K2 = MAX(SDV%K2,1)
               ENDIF
...
REAL(EB) :: AGL_SLICE
INTEGER :: N,N1,NN,NM,MESH_NUMBER,N_SLCF_O,NITER,ITER,VELO_INDEX,GEOM_INDEX,IOR,IND2,N_UNIQUE_SLCF
LOGICAL :: VECTOR,CELL_CENTERED,DEBUG,CULL_SLICE,DRY
CHARACTER(LABEL_LENGTH) :: QUANTITY,QUANTITY_TMP,SPEC_ID,PART_ID,PROP_ID,REAC_ID,SLICETYPE
...
INTEGER :: INDEX_DUM,INDEX2_DUM,Y_INDEX_DUM,Z_INDEX_DUM,PART_INDEX_DUM,REAC_INDEX_DUM,MATL_INDEX_DUM
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   ! Group the slices by the plane they lie on.  The VTKHDF slice files hold one
   ! unstructured grid per plane, with every quantity requested on that plane written as
   ! point data on it, so the plane needs a name that is the same in every mesh.

   IF (.NOT.ALLOCATED(UNIQUE_SLICE_NAMES)) ALLOCATE(UNIQUE_SLICE_NAMES(N_SLCF_O*4))
   ...
   IF (.NOT.ALLOCATED(MESHES(1)%ALL_SLICE_NAMES)) THEN
      ...
      MESHES(1)%ALL_SLICE_NAMES = ''
      MESHES(1)%ALL_SLICE_QUANTITIES = ''
      MESHES(1)%ALL_SLICE_TOPOLOGIES = -1
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      ! Register the plane this slice lies on, and the quantities requested on it, before
      ! the slice is possibly culled.  A plane is written to the VTKHDF file even when it
      ! misses this mesh, so every rank has to agree on the full list.
      ...
      VECTORLOOPNAME: DO ITER=1,NITER
         ...
         IF (PBX>-1.E5_EB .OR. PBY>-1.E5_EB .OR. PBZ>-1.E5_EB .OR. AGL_SLICE>0._EB) THEN
            MESHES(1)%ALL_SLICE_TOPOLOGIES(N1) = 0   ! planar slice
         ELSE
            MESHES(1)%ALL_SLICE_TOPOLOGIES(N1) = 1   ! 3-D slice
         ENDIF
         ...
         CALL GET_QUANTITY_INDEX(SMOKEVIEW_LABEL,SMOKEVIEW_BAR_LABEL,INDEX_DUM,INDEX2_DUM, &
                                 Y_INDEX_DUM,Z_INDEX_DUM,I_DUM(1),PART_INDEX_DUM,I_DUM(1),I_DUM(2), &
                                 REAC_INDEX_DUM,MATL_INDEX_DUM, &
                                 'SLCF',QUANTITY_TMP,'null',SPEC_ID,'null',PART_ID,'null','null',REAC_ID,'null', &
                                 -1._EB,I_DUM(3),SLICETYPE=SLICETYPE)
         MESHES(1)%ALL_SLICE_QUANTITIES(N1) = SMOKEVIEW_LABEL
      ENDDO VECTORLOOPNAME
      MESHES(1)%N_SLCF_VTK = N1

      DO I=1,N_UNIQUE_SLCF
         IF (TRIM(SLCF_NAME)==TRIM(UNIQUE_SLICE_NAMES(I))) &
            M%UNIQUE_SLICE_IS_SL3D(I) = MESHES(1)%ALL_SLICE_TOPOLOGIES(N1)==1
      ENDDO
   ...
   DO NN = 1,N_SLCF
      ...
      ENDIF
      DO I=1,N_UNIQUE_SLCF
         IF (TRIM(SL%SLCF_NAME)==TRIM(UNIQUE_SLICE_NAMES(I))) M%EMPTY_UNIQUE_SLICE(I) = .FALSE.
      ENDDO
   ENDDO
...
CHARACTER(200) :: SLCF_NAME
CHARACTER(200), DIMENSION(N_SLCF_O*4) :: UNIQUE_SLICE_NAMES
REAL(EB), DIMENSION(N_SLCF_O*4) :: AGL_SLICES
