...
INTEGER :: II,JJ,KK,IC,ICG,IOR,NOM,ITER,OBST_INDEX,OBST_INDEX_PREVIOUS,NN,NNN,NL,NLL,N_LAYERS_OBST,&
           N_MATL_OBST,N_LAYERS,N_MATLS,IIF,JJF,KKF,OBST_N_LAYER
...
REAL(EB) :: XXC,YYC,ZZC,THICKNESS,OLD_THICKNESS,FRONT_LINING_THICKNESS,BACK_LINING_THICKNESS
...
IF (SF%VARIABLE_THICKNESS .OR. SF%HT_DIM>1) THEN
   ...
   !This logic is now handled within the N_LAYERS_OBST loop
   ...
   DO NL=1,N_LAYERS_OBST
      ...
      DO NLL=1,SF%N_LAYERS
         IF (.NOT.SF%LINING) EXIT
         N_LAYERS = N_LAYERS + 1
         LAYER_THICKNESS(N_LAYERS)  = SF%LAYER_THICKNESS(NLL) 
         MINIMUM_LAYER_THICKNESS(N_LAYERS)  = SF%MINIMUM_LAYER_THICKNESS(NLL) 
         HT3D_LAYER(N_LAYERS)       = .FALSE.
         HEAT_SOURCE(N_LAYERS)      = SF%HEAT_SOURCE(NLL)
         STRETCH_FACTOR(N_LAYERS)   = SF%STRETCH_FACTOR(NLL)
         CELL_SIZE(N_LAYERS)        = SF%CELL_SIZE(NLL)
         CELL_SIZE_FACTOR(N_LAYERS) = SF%CELL_SIZE_FACTOR(NLL)
         N_LAYER_CELLS_MAX(N_LAYERS) = SF%N_LAYER_CELLS_MAX(NLL)
         DO NN=1,SF%N_LAYER_MATL(NLL)
            DO NNN=1,N_MATLS
               IF (SF%LAYER_MATL_INDEX(NLL,NN)==MATL_INDEX(NNN)) MATL_MASS_FRACTION(N_LAYERS,NNN) = SF%MATL_MASS_FRACTION(NLL,NN)
            ENDDO
         ENDDO
         LAYER_THICKNESS_OBST(NL) = LAYER_THICKNESS_OBST(NL) - LAYER_THICKNESS(N_LAYERS)
      ENDDO
      ...
      DO NN=1,N_MATL_OBST
         ...
         ENDDO
      ENDDO
      OBST_N_LAYER = N_LAYERS
      
      ! Add layers from the back surface lining
      DO NLL=1,SF_BACK%N_LAYERS
         IF (.NOT.SF_BACK%LINING) EXIT
         N_LAYERS = N_LAYERS + 1
         LAYER_THICKNESS(N_LAYERS)  = SF_BACK%LAYER_THICKNESS(SF_BACK%N_LAYERS-NLL+1)
         LAYER_THICKNESS(OBST_N_LAYER) = LAYER_THICKNESS(OBST_N_LAYER) - LAYER_THICKNESS(N_LAYERS)
         MINIMUM_LAYER_THICKNESS(N_LAYERS)  = SF_BACK%MINIMUM_LAYER_THICKNESS(SF_BACK%N_LAYERS-NLL+1)
         HT3D_LAYER(N_LAYERS)       = .FALSE.
         HEAT_SOURCE(N_LAYERS)      = SF_BACK%HEAT_SOURCE(SF_BACK%N_LAYERS-NLL+1)
         STRETCH_FACTOR(N_LAYERS)   = SF_BACK%STRETCH_FACTOR(SF_BACK%N_LAYERS-NLL+1)
         CELL_SIZE(N_LAYERS)        = SF_BACK%CELL_SIZE(SF_BACK%N_LAYERS-NLL+1)
         CELL_SIZE_FACTOR(N_LAYERS) = SF_BACK%CELL_SIZE_FACTOR(SF_BACK%N_LAYERS-NLL+1)
         N_LAYER_CELLS_MAX(N_LAYERS)= SF_BACK%N_LAYER_CELLS_MAX(SF_BACK%N_LAYERS-NLL+1)
         DO NN=1,SF_BACK%N_LAYER_MATL(NLL)
            DO NNN=1,N_MATLS
               IF (SF_BACK%LAYER_MATL_INDEX(SF_BACK%N_LAYERS-NLL+1,NN)==MATL_INDEX(NNN)) &
                  MATL_MASS_FRACTION(N_LAYERS,NNN) = SF_BACK%MATL_MASS_FRACTION(SF_BACK%N_LAYERS-NLL+1,NN)
            ENDDO
         ENDDO
      ENDDO
   ENDDO
