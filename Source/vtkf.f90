...
SUBROUTINE GET_VTK_DXPL(PLIST_ID)
   ...
   IF (VTK_DXPL_ID<0_HID_T) THEN
      ...
      CALL H5PSET_DXPL_MPIO_F(VTK_DXPL_ID, H5FD_MPIO_COLLECTIVE_F, ERROR)
   ENDIF
...
SUBROUTINE PARALLEL_INIT_F32(GROUP_ID, SNAME, CRP_LIST, RANK, DDIM, CDIM, DSET_ID, PLIST_ID, MDIM)
   ...
   IF (.NOT.LINK_EXISTS) THEN
      ...
      IF (VTK_COMPRESSION_LEVEL>0) CALL H5PSET_DEFLATE_F(CRP_LIST, VTK_COMPRESSION_LEVEL, ERROR)
...
INTEGER(IB32), DIMENSION(1) :: IDATA_OUT1(1)
INTEGER(IB32), DIMENSION(1) :: IDATA_OUT_32(1)
INTEGER, ALLOCATABLE, DIMENSION(:) :: NPOINTS_ALL
...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   ! Make the per-mesh particle counts known to every rank.  This used to be an
   ! all-to-all built from O(NMESHES*N_MPI_PROCESSES) blocking point-to-point messages,
   ! repeated at every particle output time.

   IF (N_MPI_PROCESSES>1) THEN
      ALLOCATE(NPOINTS_ALL(NMESHES))
      NPOINTS_ALL = 0
      DO NM=1,NMESHES
         IF (PROCESS(NM)==MY_RANK) NPOINTS_ALL(NM) = INT(NPOINTS(NM))
      ENDDO
      CALL MPI_ALLREDUCE(MPI_IN_PLACE,NPOINTS_ALL,NMESHES,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,IERR)
      NPOINTS(1:NMESHES) = INT(NPOINTS_ALL(1:NMESHES),IB32)
      DEALLOCATE(NPOINTS_ALL)
   ENDIF
...
INTEGER :: I,J,K,I1,I2,J1,J2,K1,K2,IQ,IQQ,IND,IND2,II,II1,II2,JJ1,JJ2,KK1,KK2, &
           Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,I_INC,J_INC,&
           IFACT,JFACT,KFACT,NX,NY,NZ,KTS,NTSL,SLICEIND
