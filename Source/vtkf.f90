...
USE DUMP, ONLY : DUMP_MESH_SPREADSHEET_OUTPUTS,GAS_PHASE_OUTPUT,SOLID_PHASE_OUTPUT,PARTICLE_OUTPUT,&
                 ...
                 COMPUTE_PARTICLE_FLUXES,CORNER_VALUE,FACE_VALUE,EDGE_VALUE,DRY
#ifdef WITH_HDF5
USE HDF5
#endif
USE MPI_F08
...
PUBLIC WRITE_VTKHDF_GEOM_FILE,&
       ...
       OPEN_VTKHDF_SERIES,OPEN_VTKHDF_BNDF,OPEN_VTKHDF_SLICE,&
       OPEN_VTKHDF_SMOKE3D,ADD_ATTRIBUTE_INT,&
       CLOSE_VTKHDF_BNDF,CLOSE_VTKHDF_SMOKE3D,CLOSE_VTKHDF_SLICE,&
       EXTEND_BNDF_VTKHDF,EXTEND_SMOKE3D_VTKHDF,EXTEND_SLICE_VTKHDF
PUBLIC DUMP_VTK_MESH_OUTPUTS_SERIES,EXCHANGE_NOBST_INFO
#endif
...
SUBROUTINE ADD_DATA_TO_SMOKE3D_VTKHDF_FILE_NOOPEN(DATANAME,DATA,NM_IN,BASE_OFFSET,FAKE_WRITE)
   ...
   INTEGER :: N_WRITTEN
   INTEGER(HSIZE_T), DIMENSION(1) :: NPOINTS_MAX_ARRAY(1), NPOINTS_TOTAL_ARRAY(1)
   INTEGER(HSIZE_T), DIMENSION(1) :: MDIM, DDIM1
   ...
   DDIM1 = (/0_HSIZE_T/)
   CALL PARALLEL_INIT_U8(HDF_SM3D_G4, TRIM(DATANAME), HDF_SM3D_CRP_LIST, 1,&
      NPOINTS_MAX_ARRAY, DDIM1, DSET_ID, HDF_SM3D_PLIST_ID, MDIM) ! Values = Timesteps
   ...
   MESH_LOOP_HDF: DO NM=1,NMESHES
      ...
      IF (.NOT.FAKE_WRITE) THEN
         NPOINTS_MAX_ARRAY(1) = INT(NPOINTS_ACCUM, HSIZE_T)+BASE_OFFSET
         NPOINTS_TOTAL_ARRAY(1) = INT(NPOINTS, HSIZE_T)
         CALL PARALLEL_WRITE_U8(1, DSET_ID,&
            HDF_SM3D_PLIST_ID, NPOINTS_MAX_ARRAY, NPOINTS_TOTAL_ARRAY, DATA)
      ELSE
         NPOINTS_MAX_ARRAY(1) = 0_HSIZE_T+BASE_OFFSET
...
SUBROUTINE INITIALIZE_VTKHDF_SLICE(FILENAME,SLCFNAME,II,NTSL)
   ...
   MESH_LOOP: DO NM=1,NMESHES
      ...
      HDF_SLCF_G1_NCELLS(II,NM) = NCELLS
      ...
      HDF_SLCF_G1_NPOINTS(II,NM) = NPOINTS
...
SUBROUTINE WRITE_VTKHDF_SLICE_DATA_FILE_NOOPEN(DATASET,NM,NCELLS,NPOINTS,DATA,&
   ...
   IF (FAKE_WRITE) THEN
      ...
      EXTENT1 = 0
      CALL PARALLEL_WRITE_F32(1, DSET_ID, PLIST_ID, START1, EXTENT1, DATA)
   ELSE
...
HDF_SLCF_G1_NCELLS = 0_HID_T
HDF_SLCF_G1_NPOINTS = 0_HID_T
NTSL = 0
UNIQUE_LOOPF: DO IQ=1,MESHES(1)%N_UNIQUE_SLCF
   SLCFNAME = MESHES(1)%UNIQUE_SLICE_NAMES(IQ)
   WRITE(FILENAME,'(A,A,A,A)') TRIM(VTK_DIR)//TRIM(CHID),'_',&
            ...
            DO K=K1,K2
               DO J=J1,J2
                  DO I=I1,I2
                     SELECT CASE(OUTPUT_QUANTITY(IND)%CELL_POSITION)
                        CASE(CELL_CENTER)
                           QQ(I,J,K,IQQ) = REAL(CORNER_VALUE(QUANTITY,B,S,I,J,K,IND),FB)
                        CASE(CELL_FACE)
                           QQ(I,J,K,IQQ) = REAL(FACE_VALUE(QUANTITY,I,J,K,IND),FB)
                        CASE(CELL_EDGE)
                           QQ(I,J,K,IQQ) = REAL(EDGE_VALUE(QUANTITY,S,I,J,K,IND),FB)
                     END SELECT
                  ENDDO
               ENDDO
            ENDDO
