...
USE VTK_FDS_INTERFACE, ONLY : WRITE_PARAVIEW_STATE_FILE,EXCHANGE_NSLICE_INFO,EXCHANGE_NPATCH_INFO
#ifdef WITH_HDF5
USE HDF5
USE VTK_FDS_INTERFACE, ONLY : WRITE_VTKHDF_GEOM_FILE,DUMP_VTK_MESH_OUTPUTS_SERIES,EXCHANGE_NOBST_INFO,&
...
#ifdef WITH_HDF5
CALL H5OPEN_F(ERROR) ! Initialize the HDF5 interface
#endif
...
! Replicate the per-mesh sizing the VTKHDF writers need on every rank

CALL EXCHANGE_NPATCH_INFO
CALL EXCHANGE_NSLICE_INFO

IF (MY_RANK==0 .AND. PARAVIEW_PROJECT) CALL WRITE_PARAVIEW_STATE_FILE(NMESHES)
...
#ifdef WITH_HDF5
! Write the VTKHDF unstructured geometry file
...
IF (.NOT.RESTART) THEN
   ...
   ENDDO
#ifdef WITH_HDF5
   IF (WRITE_VTK) CALL DUMP_VTK_MESH_OUTPUTS_SERIES(T,DT,.TRUE.)
#endif
   IF (WRITE_SMV) THEN
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
...
#ifdef WITH_HDF5
   IF (WRITE_VTK) CALL DUMP_VTK_MESH_OUTPUTS_SERIES(T,DT,.FALSE.)
#endif
   IF (WRITE_SMV) THEN
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         CALL DUMP_MESH_OUTPUTS(T,DT,NM)
...
#ifdef WITH_HDF5
IF (WRITE_VTK) THEN
   IF (VTK_KEEPOPEN) THEN
      IF (N_SMOKE3D>0) CALL CLOSE_VTKHDF_SMOKE3D()
      CALL CLOSE_VTKHDF_SLICE()
      CALL CLOSE_VTKHDF_BNDF()
      CALL CLOSE_VTKHDF_PART()
   ENDIF
   CALL H5CLOSE_F(ERROR) ! Close the HDF5 interface
ENDIF
#endif
