...
USE VTK_FDS_INTERFACE, ONLY : WRITE_VTKHDF_GEOM_FILE,DUMP_VTK_MESH_OUTPUTS_SERIES,EXCHANGE_NOBST_INFO,&
                              CLOSE_VTKHDF_BNDF, CLOSE_VTKHDF_SMOKE3D, CLOSE_VTKHDF_SLICE
#endif
...
IF (.NOT.RESTART) THEN
   ...
   IF ((VTK_HDF).AND.(WRITE_VTK)) THEN 
      CALL DUMP_VTK_MESH_OUTPUTS_SERIES(T,DT,.TRUE.)
   ENDIF
#endif
   IF (WRITE_SMV) THEN
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
...
#ifdef WITH_HDF5
   IF ((VTK_HDF).AND.(WRITE_VTK)) THEN 
      CALL DUMP_VTK_MESH_OUTPUTS_SERIES(T,DT,.FALSE.)
   ENDIF
#endif
   IF (WRITE_SMV) THEN
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
