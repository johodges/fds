...
!>
!> These outputs are independent of the Smokeview/VTK output format and are
!> therefore shared by the legacy (Smokeview) and the VTK output paths.
...
!> \brief Interpolate a cell-centered quantity to a cell corner
!>
!> Shared by the legacy (Smokeview) and the VTKHDF slice writers so that both
!> produce identical values.

REAL(EB) FUNCTION CORNER_VALUE(A,B,S,I,J,K,INDX)

REAL(EB), INTENT(IN), DIMENSION(0:,0:,0:) :: A,B,S
INTEGER, INTENT(IN) :: I,J,K,INDX
...
REAL(EB) FUNCTION FACE_VALUE(QUANTITY,I,J,K,IND)

REAL(EB), INTENT(IN), DIMENSION(0:,0:,0:) :: QUANTITY
...
REAL(EB) FUNCTION EDGE_VALUE(A,S,I,J,K,INDX)
