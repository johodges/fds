...
#ifdef WITH_HDF5
PUBLIC DUMP_VTK_MESH_OUTPUTS_SERIES
#endif
...
IF_BOUNDARY_FILES: IF ((N_BNDF>0 .AND. M%BNDF_DUMP).OR.(WRITE_VTKHDF)) THEN
...
SUBROUTINE DUMMY_VENTS(FI,N1,N2,NM)

INTEGER, INTENT(IN) :: N1,N2,FI,NM
...
INTEGER :: NM,FACE_INDEX
   ...
   MESH_LOOP: DO NM=1,NMESHES
      ...
      CALL DUMMY_VENTS(1,M%JBAR,M%KBAR,NM)
      CALL DUMMY_VENTS(2,M%JBAR,M%KBAR,NM)
      CALL DUMMY_VENTS(3,M%IBAR,M%KBAR,NM)
      CALL DUMMY_VENTS(4,M%IBAR,M%KBAR,NM)
      CALL DUMMY_VENTS(5,M%IBAR,M%JBAR,NM)
      CALL DUMMY_VENTS(6,M%IBAR,M%JBAR,NM)
...
IF (N_BNDF>0 .AND. BNDF_DUMP .OR. WRITE_VTKHDF) THEN
...
#ifdef WITH_HDF5
!> \brief Initialize SMOKE3D output in VTKHDF file
...
!> \brief Initialize BNDF output in VTKHDF file
...
!> \brief Initialize SLCF and SL3D output in VTKHDF file
...
!> \brief Write out the SMOKE3D data to VTKHDF file
!>
!> \param T Current simulation time (s)
!> \param DT Current time step size (s)
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   ALL_MESH_LOOP: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MY_RANK) THEN
         ...
         CALL POINT_TO_MESH(LOWER_MESH_INDEX)
         S3 => SMOKE3D_FILE(N)
         CALL ADD_DATA_TO_SMOKE3D_VTKHDF(S3%SMOKEVIEW_LABEL(1:30),&
      ...
      ! Pack the data into a 1-D array and send to the routine that writes the file for VTKHDF
...
#ifdef WITH_HDF5
! \brief Write contour slices, Plot3D data, or 3d slices to a VTKHDF
!>
!> \param T Current simulation time (s)
!> \param DT Current time step size (s)
   ...
   ALL_SLCF_LOOP: DO II=1,MESHES(1)%N_SLCF_VTK
      ...
      ALL_MESH_LOOP: DO NM=1,NMESHES
         ...
         IF (PROCESS(NM)/=MY_RANK) THEN
            ! Current mesh is not owned by this rank, dump empty data
            ALLOCATE(QQ_PACK(0))
            CALL ADD_DATA_TO_SLICE_VTKHDF(QTY,NM,NCELLS,NPOINTS,QQ_PACK,&
         ...
         !IF (SL%SLICETYPE=='STRUCTURED') THEN ! write out slice file using original slice file format
            ...
            ELSE
               ALLOCATE(QQ_PACK(0))
            ENDIF
            CALL ADD_DATA_TO_SLICE_VTKHDF(SL%SMOKEVIEW_LABEL(1:30),NM,NCELLS,NPOINTS,QQ_PACK,&
...
#ifdef WITH_HDF5
! \brief Dump boundary quantities into VTKHDF file
!> \param T Current simulation time (s)
!> \param DT Current time step size (s)
   ...
   DATA_FILE_LOOP: DO NF=1,N_BNDF
      ...
      IF (MESHES(NM)%N_PATCH>0) THEN
         ...
         PATCH_LOOP1: DO IP=1,N_PATCH
            ...
            CALL PACK_VTK_BNDF(T,NM,PA,BF,IND,PP,PPN,QQ_PACK)
            IF (BF%CELL_CENTERED) THEN
...
CONTAINS
   ...
   SUBROUTINE PACK_VTK_BNDF(T,NM,PA,BF,IND,PP,PPN,QQ_PACK)
      IMPLICIT NONE
      TYPE(PATCH_TYPE), POINTER, INTENT(IN) :: PA
