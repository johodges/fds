...
!                CREATE_OPEN_PART_VTKHDF,CLOSE_VTKHDF_SMOKE3D
USE HDF5
#endif
...
#ifdef WITH_HDF5
PUBLIC DUMP_VTK_MESH_OUTPUTS,DUMP_VTK_MESH_OUTPUTS_SERIES
#endif
...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   MESH_LOOP1: DO NM=1,NMESHES
      ...
      EXTENT1 = 1
      IDATA_OUT_32 = NPLIM
      CALL PARALLEL_WRITE_I32(1, DSET_ID_NPTS, PLIST_ID, START1, EXTENT1, IDATA_OUT_32)
   ...
   MESH_LOOP3: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MY_RANK) THEN
         ...
         FDATA_OUT3 = 0
         CALL PARALLEL_WRITE_F32(2, DSET_ID_PTS, PLIST_ID, START2, EXTENT2, FDATA_OUT3)
         ...
         EXTENT1 = 0
         IDATA_OUT1 = 0
         CALL PARALLEL_WRITE_I32(1, DSET_ID_PTS, PLIST_ID, START1, EXTENT1, IDATA_OUT1)
         CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
         ...
         DO NN=1,LPC%N_QUANTITIES
            ...
            EXTENT1 = 0
            FDATA_OUT3 = 0
            CALL PARALLEL_WRITE_F32(1, DSET_ID_PTS, PLIST_ID, START1, EXTENT1, FDATA_OUT3(1))
            CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
         ENDDO
         CYCLE MESH_LOOP3
      ENDIF
      ...
      EXTENT1 = NPOINTS(NM)
      CALL PARALLEL_WRITE_I32(1, DSET_ID_PTS, PLIST_ID, START1, EXTENT1, TA)
      CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
      ...
      START2(1:2) = INT((/0,NPOINTS_START/),HSIZE_T)
      EXTENT2(1:2) = INT((/3, NPOINTS(NM)/),HSIZE_T)
      CALL PARALLEL_WRITE_F32(2, DSET_ID_PTS, PLIST_ID, START2, EXTENT2, COLORS)
      CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
      ...
      DO NN=1,LPC%N_QUANTITIES
         ...
         START1 = NPOINTS_START
         EXTENT1 = NPOINTS(NM)
         CALL PARALLEL_WRITE_F32(1, DSET_ID_PTS, PLIST_ID, START1, EXTENT1, QP(1:NPP,NN))
         CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
      ENDDO
...
IF (N_SMOKE3D > 0) THEN
   ...
   CALL INITIALIZE_VTKHDF_SMOKE3D(FILENAME)
ELSE
   RETURN
ENDIF
...
CALL PARALLEL_INIT_F32(HDF_SM3D_G5, TRIM("Values"), HDF_SM3D_CRP_LIST, 1, (/1_HSIZE_T/), (/0_HSIZE_T/),&
   DSET_ID, HDF_SM3D_PLIST_ID, (/H5S_UNLIMITED_F/)) ! Values = Timesteps
VTK_T = REAL(T,FB)
...
CALL PARALLEL_INIT_I32(HDF_SM3D_G5, TRIM("ConnectivityIdOffsets"), HDF_SM3D_CRP_LIST, 1,&
   (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_SM3D_PLIST_ID, (/H5S_UNLIMITED_F/))
CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_SM3D_PLIST_ID,(/0_IB32/),1)
CALL H5DCLOSE_F(DSET_ID,ERROR)
...
CALL PARALLEL_INIT_I32(HDF_SM3D_G5, TRIM("NumberOfParts"), HDF_SM3D_CRP_LIST, 1,&
   (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_SM3D_PLIST_ID, (/H5S_UNLIMITED_F/))
CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_SM3D_PLIST_ID,(/NMESHES/),1)
CALL H5DCLOSE_F(DSET_ID,ERROR)

HDF_SM3D_COUNTER = HDF_SM3D_COUNTER + 1
CALL ADD_ATTRIBUTE_INT(HDF_SM3D_G5,(/1_HSIZE_T/),"NSteps",HDF_SM3D_COUNTER)
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   CALL PARALLEL_INIT_I32(HDF_SM3D_G7, TRIM(S3%SMOKEVIEW_LABEL(1:30)), HDF_SM3D_CRP_LIST, 1,&
      (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_SM3D_PLIST_ID, (/H5S_UNLIMITED_F/))
   CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_SM3D_PLIST_ID,INT((/BASE_OFFSET/),IB32),1)
   CALL H5DCLOSE_F(DSET_ID,ERROR)
   ...
   ALL_MESH_LOOP: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MY_RANK) THEN
         ...
         CALL ADD_DATA_TO_SMOKE3D_VTKHDF_FILE_NOOPEN(S3%SMOKEVIEW_LABEL(1:30),&
            QQ_PACK_INT, LOWER_MESH_INDEX, BASE_OFFSET, .TRUE.) !REAL(QQ_PACK, FB))
      ...
      ENDIF
      CALL ADD_DATA_TO_SMOKE3D_VTKHDF_FILE_NOOPEN(S3%SMOKEVIEW_LABEL(1:30),&
         QQ_PACK_INT, NM, BASE_OFFSET, .FALSE.) !REAL(QQ_PACK, FB))
      DEALLOCATE(QQ_PACK_INT)
      DEALLOCATE(QQ_PACK)
   ENDDO ALL_MESH_LOOP
ENDDO DATA_FILE_LOOP
...
USE TRAN, ONLY : GET_IJK
USE ISOSMOKE, ONLY: SLICE_TO_RLEFILE
INTEGER :: NM,NMNM,IFRMT,N_WRITTEN
REAL(EB), INTENT(IN) :: T,DT
REAL(EB) :: BSUM
INTEGER :: I,J,K,I1,I2,J1,J2,K1,K2,IQ,IQQ,IND,IND2,II,II1,II2,JJ1,JJ2,KK1,KK2, &
           IC,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,NOM,IIO,JJO,KKO,I_INC,J_INC,&
...
UNIQUE_LOOPF: DO IQ=1,MESHES(1)%N_UNIQUE_SLCF
   ...
   ! Write time step Information
   CALL PARALLEL_INIT_F32(HDF_SLCF_G5(IQ), TRIM("Values"), HDF_SLCF_CRP_LIST(IQ), 1, (/1_HSIZE_T/), (/0_HSIZE_T/),&
      DSET_ID, HDF_SLCF_PLIST_ID(IQ), (/H5S_UNLIMITED_F/)) ! Values = Timesteps
      VTK_T = REAL(T,FB)
   ...
   CALL PARALLEL_INIT_I32(HDF_SLCF_G5(IQ), TRIM("ConnectivityIdOffsets"), HDF_SLCF_CRP_LIST(IQ), 1,&
      (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_SLCF_PLIST_ID(IQ), (/H5S_UNLIMITED_F/))
   CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_SLCF_PLIST_ID(IQ),(/0_IB32/),1)
   CALL H5DCLOSE_F(DSET_ID,ERROR)
   ...
   CALL PARALLEL_INIT_I32(HDF_SLCF_G5(IQ), TRIM("NumberOfParts"), HDF_SLCF_CRP_LIST(IQ), 1,&
      (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_SLCF_PLIST_ID(IQ), (/H5S_UNLIMITED_F/))
   CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_SLCF_PLIST_ID(IQ),(/NMESHES/),1)
   CALL H5DCLOSE_F(DSET_ID,ERROR)

   HDF_SLCF_COUNTER(IQ) = HDF_SLCF_COUNTER(IQ) + 1
   CALL ADD_ATTRIBUTE_INT(HDF_SLCF_G5(IQ),(/1_HSIZE_T/),"NSteps",HDF_SLCF_COUNTER(IQ))
   ...
   ALL_SLCF_LOOP: DO II=1,MESHES(1)%N_SLCF_VTK
      ...
      CALL EXTEND_SLICE_VTKHDF(QTY, II, BASE_OFFSET_PTS,NCELLS,NPOINTS)
      !WRITE(*,*) MY_RANK, MESHES(1)%ALL_SLICE_NAMES(II), QTY, BASE_OFFSET_PTS
      
      ! Write point data offsets.
      CALL PARALLEL_INIT_I32(HDF_SLCF_G7(II), TRIM(QTY), HDF_SLCF_CRP_LIST(II), 1,&
         (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_SLCF_PLIST_ID(II), (/H5S_UNLIMITED_F/))
      CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_SLCF_PLIST_ID(II),INT((/BASE_OFFSET_PTS/),IB32),1)
      CALL H5DCLOSE_F(DSET_ID,ERROR)
      ...
      ALL_MESH_LOOP: DO NM=1,NMESHES
         ...
         IF (PROCESS(NM)/=MY_RANK) THEN
            ...
            ! Write fake data in processes that have less meshes than max process
            !ALLOCATE(QQ_PACK(0))
            !N_WRITTEN = 0 !MESHES_PER_PROCESS(MY_RANK)-1
            !DO NMNM=1,MAXVAL(MESHES_PER_PROCESS)
            !   IF (N_WRITTEN < MAXVAL(MESHES_PER_PROCESS)) THEN
            !      CALL WRITE_VTKHDF_SLICE_DATA_FILE_NOOPEN(QTY,NM,NCELLS,NPOINTS,QQ_PACK,&
            !         HDF_SLCF_PLIST_ID(IQ),HDF_SLCF_CRP_LIST(IQ),HDF_SLCF_G4(IQ),BASE_OFFSET_PTS,.TRUE.)
            !      N_WRITTEN = N_WRITTEN + 1
         ...
         !IF (SL%SLICETYPE=='STRUCTURED') THEN ! write out slice file using original slice file format
            ...
            CALL WRITE_VTKHDF_SLICE_DATA_FILE_NOOPEN(SL%SMOKEVIEW_LABEL(1:30),NM,NCELLS,NPOINTS,QQ_PACK,&
               HDF_SLCF_PLIST_ID(IQ),HDF_SLCF_CRP_LIST(IQ),HDF_SLCF_G4(IQ),BASE_OFFSET_PTS,.FALSE.)
            DEALLOCATE(QQ_PACK)
            N_WRITTEN = N_WRITTEN + 1
         !ENDIF
      ENDDO ALL_MESH_LOOP
...
! Write time step Information
CALL PARALLEL_INIT_F32(HDF_BNDF_G5, TRIM("Values"), HDF_BNDF_CRP_LIST, 1, (/1_HSIZE_T/), (/0_HSIZE_T/),&
   DSET_ID, HDF_BNDF_PLIST_ID, (/H5S_UNLIMITED_F/)) ! Values = Timesteps
VTK_T = REAL(T,FB)
...
CALL PARALLEL_INIT_I32(HDF_BNDF_G5, TRIM("ConnectivityIdOffsets"), HDF_BNDF_CRP_LIST, 1,&
   (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_BNDF_PLIST_ID, (/H5S_UNLIMITED_F/))
CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_BNDF_PLIST_ID,(/0_IB32/),1)
CALL H5DCLOSE_F(DSET_ID,ERROR)
...
CALL PARALLEL_INIT_I32(HDF_BNDF_G5, TRIM("NumberOfParts"), HDF_BNDF_CRP_LIST, 1,&
   (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_BNDF_PLIST_ID, (/H5S_UNLIMITED_F/))
CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_BNDF_PLIST_ID,(/2*NMESHES/),1)
CALL H5DCLOSE_F(DSET_ID,ERROR)

HDF_BNDF_COUNTER = HDF_BNDF_COUNTER + 1
CALL ADD_ATTRIBUTE_INT(HDF_BNDF_G5,(/1_HSIZE_T/),"NSteps",HDF_BNDF_COUNTER)
...
DO NF=1,N_BNDF
   ...
   IF (BF%CELL_CENTERED) THEN
      CALL PARALLEL_INIT_I32(HDF_BNDF_G6, TRIM(BF%SMOKEVIEW_LABEL(1:30)), HDF_BNDF_CRP_LIST, 1,&
         (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_BNDF_PLIST_ID, (/H5S_UNLIMITED_F/))
      CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_BNDF_PLIST_ID,INT((/BASE_OFFSET_CELLS/),IB32),1)
   ELSE
      CALL PARALLEL_INIT_I32(HDF_BNDF_G7, TRIM(BF%SMOKEVIEW_LABEL(1:30)), HDF_BNDF_CRP_LIST, 1,&
         (/1_HSIZE_T/), (/0_HSIZE_T/), DSET_ID, HDF_BNDF_PLIST_ID, (/H5S_UNLIMITED_F/))
      CALL APPEND_RANK1_DATASET_I32(DSET_ID,HDF_BNDF_PLIST_ID,INT((/BASE_OFFSET_PTS/),IB32),1)
   ENDIF
   CALL H5DCLOSE_F(DSET_ID,ERROR)
ENDDO
      ...
      IF (MESHES(NM)%N_PATCH>0) THEN
         ...
         IF (BF%CELL_CENTERED) THEN
            ...
            CALL PARALLEL_INIT_F32(HDF_BNDF_G2, BF%SMOKEVIEW_LABEL(1:30), HDF_BNDF_CRP_LIST, 1,&
               START1, EXTENT1, DSET_ID, HDF_BNDF_PLIST_ID) ! Open previously initialized cell centered for writing
            START1=NCELLS_START + BASE_OFFSET_CELLS
            EXTENT1=NCELLS(NM1)
            CALL PARALLEL_WRITE_F32(1, DSET_ID, HDF_BNDF_PLIST_ID, START1, EXTENT1, ALL_DATA)
         ELSE
            START1=0
            EXTENT1=0
            CALL PARALLEL_INIT_F32(HDF_BNDF_G4, BF%SMOKEVIEW_LABEL(1:30), HDF_BNDF_CRP_LIST, 1,&
               START1, EXTENT1, DSET_ID, HDF_BNDF_PLIST_ID) ! Open previously initialized nodal for writing
            START1=NPOINTS_START + BASE_OFFSET_PTS
            EXTENT1=NPOINTS(NM1)
            CALL PARALLEL_WRITE_F32(1, DSET_ID, HDF_BNDF_PLIST_ID, START1, EXTENT1, ALL_DATA)
         ENDIF
         CALL H5DCLOSE_F(DSET_ID, ERROR)
         DEALLOCATE(ALL_DATA)
         IF (NF==1) N_WRITTEN = N_WRITTEN + 1
      ENDIF
      ...
      IF (MESHES(NM)%N_INTERNAL_CFACE_CELLS>0) THEN
         ...
         IF (NVERTS>0 .AND. NFACES>0) THEN
            ...
            IF (BF%CELL_CENTERED) THEN
               ...
               CALL PARALLEL_INIT_F32(HDF_BNDF_G2, BF%SMOKEVIEW_LABEL(1:30), HDF_BNDF_CRP_LIST, 1,&
                  START1, EXTENT1, DSET_ID, HDF_BNDF_PLIST_ID) ! Open previously initialized cell centered for writing
               START1=NCELLS_ACCUM + BASE_OFFSET_CELLS
               EXTENT1=NCELLS(NM2)
               CALL PARALLEL_WRITE_F32(1, DSET_ID, HDF_BNDF_PLIST_ID, START1, EXTENT1, ALL_DATA)
            ELSE
               START1=0
               ...
               CALL PARALLEL_INIT_F32(HDF_BNDF_G4, BF%SMOKEVIEW_LABEL(1:30), HDF_BNDF_CRP_LIST, 1,&
                  START1, EXTENT1, DSET_ID, HDF_BNDF_PLIST_ID) ! Open previously initialized nodal for writing
               START1=NPOINTS_ACCUM + BASE_OFFSET_PTS
               EXTENT1=NPOINTS(NM2)
               CALL PARALLEL_WRITE_F32(1, DSET_ID, HDF_BNDF_PLIST_ID, START1, EXTENT1, ALL_DATA)
            ENDIF
            CALL H5DCLOSE_F(DSET_ID, ERROR)
...
DO NMNM2=1,MAXVAL(MESHES_PER_PROCESS)*2
   IF (N_WRITTEN < MAXVAL(MESHES_PER_PROCESS)*2) THEN
      DATA_FILE_LOOP2: DO NF=1,N_BNDF
         BF => BOUNDARY_FILE(NF)
         PY => PROPERTY(BF%PROP_INDEX)
         IF (BF%CELL_CENTERED) THEN
            CALL PARALLEL_INIT_F32(HDF_BNDF_G2, BF%SMOKEVIEW_LABEL(1:30), HDF_BNDF_CRP_LIST, 1,&
               CDIM1,DDIM1, DSET_ID, HDF_BNDF_PLIST_ID) ! Open previously initialized cell centered for writing
            CALL PARALLEL_WRITE_F32(1, DSET_ID, HDF_BNDF_PLIST_ID, CDIM1,DDIM1, RDATA_OUT3)
         ELSE
            CALL PARALLEL_INIT_F32(HDF_BNDF_G4, BF%SMOKEVIEW_LABEL(1:30), HDF_BNDF_CRP_LIST, 1,&
               CDIM1,DDIM1, DSET_ID, HDF_BNDF_PLIST_ID) ! Open previously initialized nodal for writing
            CALL PARALLEL_WRITE_F32(1, DSET_ID, HDF_BNDF_PLIST_ID, CDIM1,DDIM1, RDATA_OUT3)
         ENDIF
         CALL H5DCLOSE_F(DSET_ID, ERROR)
