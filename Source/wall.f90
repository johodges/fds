...
B1%ZZ_G(1:N_TOTAL_SCALARS) = ZZP(BC%IIG,BC%JJG,BC%KKG,1:N_TOTAL_SCALARS)
...
REAL(EB) :: ARO,QNET,RAMP_FACTOR,RHO_G_2,RSUM_F,PBAR_F,TMP_OTHER_SOLID,TSI,UN, &
            RHO_ZZ_F(1:N_TOTAL_SCALARS),ZZ_GET(1:N_TOTAL_SCALARS), &
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (INFLOW_OUTFLOW) METHOD_OF_HEAT_TRANSFER  ! Only for WALL cells
      ...
      IF (UN>TWO_EPSILON_EB) THEN  ! Assume the flow is coming into the domain
         ...
         IF (MIXING_AS_PASSIVE_SCALAR_TRANSPORT) THEN
            MASS1 = DX(BC%IIG)*DY(BC%JJG)*DZ(BC%KKG)*RHO(BC%IIG,BC%JJG,BC%KKG)
            MASS_IN = B1%AREA*UN*RHO(BC%IIG,BC%JJG,BC%KKG)
            B1%ZZ_F(ZETA_INDEX) = (SPECIES_MIXTURE(ZETA_INDEX)%ZZ0 * MASS1 + 1._EB*MASS_IN) / (MASS1+MASS_IN)
         ENDIF
      ELSE
         B1%TMP_F = B1%TMP_G
         B1%ZZ_F(1:N_TRACKED_SPECIES) = B1%ZZ_G(1:N_TRACKED_SPECIES)
         IF (MIXING_AS_PASSIVE_SCALAR_TRANSPORT) B1%ZZ_F(ZETA_INDEX) = B1%ZZ_G(ZETA_INDEX)
      ENDIF
      ...
      IF (MIXING_AS_PASSIVE_SCALAR_TRANSPORT) THEN
         ZZP(BC%II,BC%JJ,BC%KK,N_TRACKED_SPECIES+1:N_TOTAL_SCALARS) = B1%ZZ_F(N_TRACKED_SPECIES+1:N_TOTAL_SCALARS)
         ZZ_GET(N_TRACKED_SPECIES+1:N_TOTAL_SCALARS) = MAX(0._EB,ZZP(BC%II,BC%JJ,BC%KK,N_TRACKED_SPECIES+1:N_TOTAL_SCALARS))
      ENDIF
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET(1:N_TRACKED_SPECIES),RSUM(BC%II,BC%JJ,BC%KK))
   ...
   CASE (INTERPOLATED_BC) METHOD_OF_HEAT_TRANSFER  ! Only for EXTERNAL_WALL_CELLs
      ...
      IF (ATMOSPHERIC_INTERPOLATION) THEN
         ...
         ZZ_GET(1:N_TOTAL_SCALARS) = MAX(0._EB,MIN(1._EB,RHO_ZZ_OTHER(1:N_TOTAL_SCALARS)/RHO_OTHER))
         CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET(1:N_TRACKED_SPECIES),RSUM(BC%II,BC%JJ,BC%KK))
      ...
      SINGLE_SPEC_IF: IF (N_TOTAL_SCALARS > 1) THEN
         SPECIES_LOOP: DO N=1,N_TOTAL_SCALARS
            ...
            SELECT CASE(BC%IOR)
               CASE( 1)
                  IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
                     RHO_ZZ_G_2 = ZZP(BC%IIG+1,BC%JJG,BC%KKG,N)
                     IF (N > N_TRACKED_SPECIES) RHO_ZZ_G_2 = RHOP(BC%IIG+1,BC%JJG,BC%KKG)*RHO_ZZ_G_2
                     RHO_ZZ_OTHER_2 = OM_ZZP(IIO-1,JJO,KKO,N)
                     IF (N > N_TRACKED_SPECIES) RHO_ZZ_OTHER_2 = OM_RHOP(IIO-1,JJO,KKO)*RHO_ZZ_OTHER_2
                  ENDIF
               ...
               CASE( 2)
                  IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
                     RHO_ZZ_G_2 = ZZP(BC%IIG,BC%JJG+1,BC%KKG,N)
                     IF (N > N_TRACKED_SPECIES) RHO_ZZ_G_2 = RHOP(BC%IIG,BC%JJG+1,BC%KKG)*RHO_ZZ_G_2
                     RHO_ZZ_OTHER_2 = OM_ZZP(IIO,JJO-1,KKO,N)
                     IF (N > N_TRACKED_SPECIES) RHO_ZZ_OTHER_2 = OM_RHOP(IIO,JJO-1,KKO)*RHO_ZZ_OTHER_2
                  ENDIF
               ...
               CASE( 3)
                  IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
                     RHO_ZZ_G_2 = ZZP(BC%IIG,BC%JJG,BC%KKG+1,N)
                     IF (N > N_TRACKED_SPECIES) RHO_ZZ_G_2 = RHOP(BC%IIG,BC%JJG,BC%KKG+1)*RHO_ZZ_G_2
                     RHO_ZZ_OTHER_2 = OM_ZZP(IIO,JJO,KKO-1,N)
                     IF (N > N_TRACKED_SPECIES) RHO_ZZ_OTHER_2 = OM_RHOP(IIO,JJO,KKO-1)*RHO_ZZ_OTHER_2
                  ENDIF
         ...
         ZZP(BC%II,BC%JJ,BC%KK,1:N_TRACKED_SPECIES) = MAX(0._EB,MIN(1._EB,RHO_ZZ_OTHER(1:N_TRACKED_SPECIES)/RHO_OTHER))
         ZZ_GET(1:N_TRACKED_SPECIES) = ZZP(BC%II,BC%JJ,BC%KK,1:N_TRACKED_SPECIES)
         IF (MIXING_AS_PASSIVE_SCALAR_TRANSPORT) THEN
            ZZP(BC%II,BC%JJ,BC%KK,ZETA_INDEX) = RHO_ZZ_OTHER(ZETA_INDEX)/RHO_OTHER
            B1%ZZ_F(ZETA_INDEX) = ZZP(BC%II,BC%JJ,BC%KK,ZETA_INDEX)
            ZZ_GET(ZETA_INDEX) = ZZP(BC%II,BC%JJ,BC%KK,ZETA_INDEX)
         ENDIF
         CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET(1:N_TRACKED_SPECIES),RSUM(BC%II,BC%JJ,BC%KK))
         ...
         IF (ATMOSPHERIC_INTERPOLATION) THEN
            ...
            CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET(1:N_TRACKED_SPECIES),RSUM_F)
            B1%RHO_F = PBAR_F/(RSUM_F*B1%TMP_F)
         ELSE
            B1%ZZ_F(1:N_TRACKED_SPECIES) = MAX(0._EB,MIN(1._EB,RHO_ZZ_F(1:N_TRACKED_SPECIES)/B1%RHO_F))
            IF (MIXING_AS_PASSIVE_SCALAR_TRANSPORT) B1%ZZ_F(ZETA_INDEX) = ZZP(BC%II,BC%JJ,BC%KK,ZETA_INDEX)
            ZZ_GET(1:N_TRACKED_SPECIES) = B1%ZZ_F(1:N_TRACKED_SPECIES)
            CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET(1:N_TRACKED_SPECIES),RSUM_F)
                     ...
                     IIO_LOOP: DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                        ...
                        MODE_SELECT: SELECT CASE(SIM_MODE)
                           ...
                           CASE(DNS_MODE,LES_MODE)
                              ...
                              CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET(1:N_TRACKED_SPECIES),RSUM_OTHER)
                              ...
                              IF (SIM_MODE==LES_MODE) THEN
                                 CALL GET_VISCOSITY(ZZ_GET(1:N_TRACKED_SPECIES),MU_DNS_OTHER,TMP_OTHER)
                                 RSC_LOC = RSC
...
REAL(EB) :: RVC,M_DOT_PPP_SINGLE,ZZ_GET(1:N_TOTAL_SCALARS),DENOM,MASS1,MASS_IN
...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (NO_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      B1%ZZ_F(1:N_TOTAL_SCALARS) = B1%ZZ_G(1:N_TOTAL_SCALARS)
   ...
   CASE (SPECIFIED_MASS_FRACTION) METHOD_OF_MASS_TRANSFER
      ...
      IF (B1%U_NORMAL_S<0._EB) THEN  ! If there is a non-zero velocity into the domain, assign appropriate species
         ...
         CALL GET_REALIZABLE_MF(ZZ_GET(1:N_TRACKED_SPECIES))
         IF (N_PASSIVE_SCALARS >0) ZZ_GET(N_TRACKED_SPECIES+1:N_TOTAL_SCALARS) = B1%ZZ_G(N_TRACKED_SPECIES+1:N_TOTAL_SCALARS)
         B1%ZZ_F = ZZ_GET
      ELSE
         B1%ZZ_F(1:N_TOTAL_SCALARS) = B1%ZZ_G(1:N_TOTAL_SCALARS)
      ENDIF
      ...
      ENDDO
      ! Define zeta boundary condition for specified mass fraction
      IF (MIXING_AS_PASSIVE_SCALAR_TRANSPORT) THEN
         MASS1 = DX(BC%IIG)*DY(BC%JJG)*DZ(BC%KKG)*RHO(BC%IIG,BC%JJG,BC%KKG)
         MASS_IN = SUM(B1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES))*B1%AREA
         IF (MASS_IN > 0._EB) THEN
            ZZ(BC%IIG,BC%JJG,BC%KKG,ZETA_INDEX) = (ZZ(BC%IIG,BC%JJG,BC%KKG,ZETA_INDEX) * MASS1 + 1._EB*MASS_IN) / (MASS1+MASS_IN)
            B1%ZZ_F(ZETA_INDEX) = ZZ(BC%IIG,BC%JJG,BC%KKG,ZETA_INDEX)
            B1%ZZ_G(ZETA_INDEX) = ZZ(BC%IIG,BC%JJG,BC%KKG,ZETA_INDEX)
         ENDIF
      ENDIF
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      ! Define zeta boundary condition for pyrolyzate
      IF (MIXING_AS_PASSIVE_SCALAR_TRANSPORT) THEN
         MASS1 = DX(BC%IIG)*DY(BC%JJG)*DZ(BC%KKG)*RHO(BC%IIG,BC%JJG,BC%KKG)
         MASS_IN = SUM(B1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES))*B1%AREA
         ZZ(BC%IIG,BC%JJG,BC%KKG,ZETA_INDEX) = (ZZ(BC%IIG,BC%JJG,BC%KKG,ZETA_INDEX) * MASS1 + 1._EB*MASS_IN) / (MASS1+MASS_IN)
      ...
      ELSEIF (B1%LAYER_REMOVED .OR. (PRESENT(WALL_INDEX) .AND. .NOT.CELL(IC)%SOLID .AND. .NOT.CELL(IC)%EXTERIOR)) THEN
         ...
         IF (CORRECTOR) THEN  ! calculate the mass production rate of gases in the adjacent gas cell
            ...
            CALL GET_SPECIFIC_HEAT(ZZ_GET(1:N_TRACKED_SPECIES),CP,B1%TMP_G)
            H_G = CP*B1%TMP_G
            !$OMP CRITICAL
...
IF (PRESENT(WALL_INDEX)) THEN
   IF (WALL_INDEX<=N_EXTERNAL_WALL_CELLS .AND. .NOT.CELL(IC)%SOLID .AND. .NOT.CELL(ICG)%SOLID) &
       ZZP(BC%II,BC%JJ,BC%KK,1:N_TOTAL_SCALARS) = 2._EB*B1%ZZ_F(1:N_TOTAL_SCALARS) - B1%ZZ_G(1:N_TOTAL_SCALARS)
ENDIF
