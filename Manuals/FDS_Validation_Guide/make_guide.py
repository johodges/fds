...
valguide_aux = os.path.join(firemodels, 'fds','Manuals','FDS_Validation_Guide', 'FDS_User_Guide.aux')

if os.path.exists(userguide_aux) is False:
    print("***warning: $AUXUSER does not exist. Build the FDS User's")
    print("            Guide before building the FDS Validation Guide")
else:
    shutil.copy2(userguide_aux, valguide_aux)
